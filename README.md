stcflash 是基于 stcgal 的二次开发！Stcgal 原始项目地址：https://github.com/grigorig/stcgal
stcflash is a secondary development based on stcgal! Stcgal original project address: https://github.com/grigorig/stcgal


失败的逻辑 如果你们可以 研究出来 理论上支持所有的MCU（不限于STC，STM，ESP等等），本项目支持你们提交代码修改完善这个项目！文件地址：https://wwanr.lanzouw.com/b00mq2gzoh
密码:gu10

If you can work out the logic of failure that theoretically supports all MCUS (not limited to STC, STM, ESP, etc.), this project supports you to submit code to modify and improve this project! The file address: https://wwanr.lanzouw.com/b00mq2gzoh
Password :gu10
...
STC 微控制器具有基于 UART/USB 的引导程序加载器 (BSL)。它使用基于数据包的协议通过串行链接对代码内存和 IAP 内存进行编程。这被称为在线系统编程 (ISP)。BSL 还用于配置各种（类似保险丝的）设备选项。遗憾的是，该协议未公开，STC 仅提供了一个（粗糙的）Windows GUI 应用程序用于编程。
...
stcflash 是 STC Windows 软件的全功能开源替代品；它支持范围广泛的 MCU，具有很强的便携性，适合自动化使用。
stcflash is a full-featured Open Source replacement for STC's Windows software; it supports a wide range of MCUs, it is very portable and suitable for automation.
...
[如何使用 stcflash](doc/USAGE.md) / [How to use stcflash](doc/USAGE.md)

[常见问题](doc/FAQ.md) / [Frequently Asked Questions](doc/FAQ.md)

[测试过的 MCU 型号列表](doc/MODELS.md) / [List of tested MCU models](doc/MODELS.md)
